...
// Restored from git e78665f (deleted in 3d3a14c) for the /dev/effects-lab
// preview. Identical rendering logic; params now arrive as a prop so the
// lab's control panel can drive them live.

import { useRef, useEffect, useCallback, useState } from "react";

export type DotShape = "circle" | "square" | "diamond" | "cross" | "line";
...
// (components/BackgroundTexture.tsx) — keep the two in sync.
...
export function BackgroundTexture({ params }: { params: TextureParams }) {
  ...
  // Refresh colors when the panel changes color settings
  const { useCustomColors, dotColor, glowColor } = params.colors;
  useEffect(() => {
    if (!mounted) return;
    refreshColors();
  }, [mounted, useCustomColors, dotColor, glowColor, refreshColors]);
