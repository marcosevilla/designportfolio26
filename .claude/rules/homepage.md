...
- **Playground cards run at the TAIL of the same track (2026-08-15).** The separate "Just for fun" section below the strip is DELETED — Marco's call, so the page reads as one body of work instead of client-work-then-sidequests. Work cards stay first: a reviewer who sees only two cards sees two Canary studies. `PlaygroundMarqueeCell` in `CaseStudyList.tsx` renders them from `lib/playground-cards.ts`.
  - ⚠️ **They MUST keep the identical `w-[520px] max-w-[80vw]` cell class.** The arrows measure their step off the first `.work-marquee-cell` and assume a uniform stride — a differently-sized playground cell would make one press travel the wrong distance. Verified 544px stride across the work→fun boundary at 1440, 371px at 390.
  - **What they deliberately DON'T inherit:** no `DitherBackdrop` (nine always-on WebGL canvases already hold the GPU process ~53% with no IntersectionObserver — four more to paint a backdrop the landscape videos fully cover was a bad trade; the quieter 4% `FRAME_BG` wash stands in), no `CursorGlowOverlay` (playground cells never had the rim glow), no `LockGate`.
  - **Portrait captures are PILLARBOXED, not cropped** — `object-fit: contain` when `isWide(card)` is false. `cover` on a 13/10 frame reduces a 628×1080 phone capture to a sliver and ate Pajamagrams' title in the old stacked layout. Landscape cards keep `cover`. Trade-off: a contained portrait clip fills only ~45% of the 520×400 frame, so those two cards read lighter than the rest of the strip.
  - **The mono meta line is the only sidequest signal** now that the header is gone: `PERSONAL • JUL 2026` vs a study's `CANARY TECHNOLOGIES • 2026`. ⚠️ Note `ai-workflow` is a *work* card that also reads "Personal", so the signal is soft.
  - **Cards without an `href` render as an inert `<div>`** (no pointer, no focus ring) rather than a dead link among clickable cards. Pajamagrams + Custom Wrapped are undeployed; tracked in Todoist.
- **`SectionLabel` was DELETED 2026-08-15** along with the playground section — it had zero consumers left (the marquee dropped its own label 2026-07-20). Recover from git if a section header is ever wanted.
