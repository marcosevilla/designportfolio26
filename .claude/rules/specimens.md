...
- **Panel: phone only (Marco 2026-08-05, second pass).** DemoStage itself renders no panel. The guest phone specimen gets one back — applied at the *call site* in `FBOrderingContent.tsx`, not in DemoStage: `rounded-[10px] border border-border px-4 py-12 sm:py-16` over `DEMO_PANEL_BG` (`color-mix(in srgb, var(--color-fg) 7%, var(--color-bg))`, the pre-chrome-out panel color). The three staff specimens stay panel-less.
- **Every demo sits on `CONTENT_BAND` (Marco 2026-08-05, third pass).** The staff specimens used to break out to `md="1-12" lg="3-10"`; they don't any more. A 1177px staff UI renders at **0.574** in the 676px desktop band — deliberately small, it reads as a figure on the page rather than something you operate. `ObjectFlowDiagram` is now the ONLY wide-band item on the page.
- **Staff shells carry `SHELL_SHADOW` from `admin-shell.tsx`, never `ELEV.lg`** (Marco 2026-08-05). `ELEV.lg` is the product's UI-element shadow — tight and 10% at 28px — and on a 1177px window it pooled around the `RADIUS.lg` corners as a dark backing plate. `SHELL_SHADOW` is the same wide 3-stop artifact lift the guest phone and DeviceShell use, so all four demos read as one family; keep the two copies in sync. It's authored on the shell, so it scales with the inline fit-scale — a smaller artifact casts a smaller shadow, which is why it isn't scale-compensated (fullscreen renders near 1× and would look heavy). Invisible on the dark theme, same as the phone's. Applies to all three staff shells — change them together.
- **Two surfaces, two jobs (Marco 2026-08-05):** the **inline** copy is DISPLAY ONLY — auto-plays, loops, no panel background, `pointer-events: none` + `inert` (keyboard can't tab into it). The **fullscreen** copy is MANUAL ONLY — no ghost cursor, no auto-play; the visitor drives the prototype. There is no takeover path in the page; the old hover scrim + "Interact with flow" pill are gone.
...
- **The "Try demo" pill (Marco 2026-08-05, third pass)** is the only inline chrome: a dark pill that fades in over the stage's **top-right corner on hover** and opens fullscreen. It carries its own drop shadow + backdrop blur because it floats over live product UI. It hangs off the `<section class="demo-stage-root">`, NOT the scroll well — a panning well would carry it out of view — and a computed `stageInset` walks it in to the stage's own right edge when the stage is narrower than its column. **`pillAnchor="container"`** moves it to the caller's wrapper instead, which is what the guest phone uses so the pill lands in the PANEL's corner rather than on the phone's screen; the caller must then put `demo-stage-root relative` on that wrapper — `demo-stage-root` included, or the pointer crosses unhovered panel padding on its way to the pill and it hides mid-travel. Reveal/hide is CSS (`.demo-stage-root:hover`, `.demo-try-pill:focus-visible`, `@media (hover: none)` pins it visible so touch devices can open a demo at all) — see `globals.css`. ⚠️ The `(hover: none)` branch is verified as a rule, not on a real touch device.
- **`DemoGroup` / `TryDemoButton` are GONE** (deleted 2026-08-05 with the hover pill). The `DemoLaunchContext` ref bridge they needed went with them — DemoStage passes `open` straight to its inline `StageCore` as `onLaunch`. Captions are now plain `<h3>` + `<p>`, no CTA.
...
- **Inline fit-scale: fits the column exactly at viewport ≥ `PAN_BELOW` (1200), floors at `MIN_INLINE_SCALE` (0.7) and pans below it.** The floor keys off **`window.innerWidth`, not the well** — the well is the text band at every width, so keying off it floors the stage on desktop too (got this wrong once). 1200 matches the editorial grid's desktop breakpoint; below it the band is too narrow for fitting a 1177px UI to mean anything (467px at 1024 ⇒ 0.40 ⇒ 11px type at 4px). The well is **block + `margin-inline: auto`**, NOT flex centering: flex `items-center` pushes overflow off BOTH edges and strands the left half unreachable.
- **The well is `overflow-x: visible` unless it's actually panning** (`panning` state, same layout effect). An `overflow-x: auto` box computes overflow-**y** to `auto` as well, which sheared the phone shell's ambient drop shadow off flat at the stage box. Don't reintroduce an unconditional `overflow-x: auto`.
